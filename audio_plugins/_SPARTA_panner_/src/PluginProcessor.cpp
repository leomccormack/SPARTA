...
juce::AudioProcessorValueTreeState::ParameterLayout PluginProcessor::createParameterLayout()
{
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>("spread", "Spread", juce::NormalisableRange<float>(PANNER_SPREAD_MIN_VALUE, PANNER_SPREAD_MAX_VALUE, 0.01f), 0.0f, AudioParameterFloatAttributes().withAutomatable(false)));
    params.push_back(std::make_unique<juce::AudioParameterFloat>("roomCoeff", "RoomCoeff", juce::NormalisableRange<float>(0.0f, 1.0f, 0.01f), 0.0f, AudioParameterFloatAttributes().withAutomatable(false)));
    ...
    for(int i=0; i<MAX_NUM_INPUTS; i++){
        params.push_back(std::make_unique<juce::AudioParameterFloat>("srcAzim" + juce::String(i), "SrcAzim_" + juce::String(i+1), juce::NormalisableRange<float>(-180.0f, 180.0f, 0.01f), 0.0f));
        params.push_back(std::make_unique<juce::AudioParameterFloat>("srcElev" + juce::String(i), "SrcElev_" + juce::String(i+1), juce::NormalisableRange<float>(-90.0f, 90.0f, 0.01f), 0.0f));
    }
    for(int i=0; i<MAX_NUM_OUTPUTS; i++){
        params.push_back(std::make_unique<juce::AudioParameterFloat>("lsAzim" + juce::String(i), "LsAzim_" + juce::String(i+1), juce::NormalisableRange<float>(-180.0f, 180.0f, 0.01f), 0.0f, AudioParameterFloatAttributes().withAutomatable(false)));
        params.push_back(std::make_unique<juce::AudioParameterFloat>("lsElev" + juce::String(i), "LsElev_" + juce::String(i+1), juce::NormalisableRange<float>(-90.0f, 90.0f, 0.01f), 0.0f, AudioParameterFloatAttributes().withAutomatable(false)));
    }

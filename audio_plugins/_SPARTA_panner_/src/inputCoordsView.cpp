...
inputCoordsView::inputCoordsView (PluginProcessor& p, int _maxNCH, int _currentNCH ) : processor(p)
{
    ...
    for( int i=0; i<maxNCH; i++){
        ...
        aziSliders[i] = std::make_unique<SliderWithAttachment>(p.parameters, "srcAzim" + juce::String(i));
        ...
        elevSliders[i] = std::make_unique<SliderWithAttachment>(p.parameters, "srcElev" + juce::String(i));

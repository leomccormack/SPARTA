...
static juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout()
{
    ...
    for(int i=0; i<MAX_NUM_INPUTS; i++){
        params.push_back(std::make_unique<juce::AudioParameterFloat>("azim" + juce::String(i), "Azim_" + juce::String(i+1), juce::NormalisableRange<float>(-360.0f, 360.0f, 0.01f), array2sh_defaultSensorsDirections[i][0], AudioParameterFloatAttributes().withLabel(juce::String::fromUTF8(u8"\u00B0")).withAutomatable(false)));
        params.push_back(std::make_unique<juce::AudioParameterFloat>("elev" + juce::String(i), "Elev_" + juce::String(i+1), juce::NormalisableRange<float>(-180.0f, 180.0f, 0.01f), array2sh_defaultSensorsDirections[i][1], AudioParameterFloatAttributes().withLabel(juce::String::fromUTF8(u8"\u00B0")).withAutomatable(false)));
    }
...
void PluginProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    blockAdapter->configure(array2sh_getFrameSize(), nNumInputs, nNumOutputs, nHostBlockSize);
}
...
void PluginProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& /*midiMessages*/)
{
    ...
    blockAdapter->processBlock (buffer, [this] (const float* const* inFrame, float* const* outFrame, int numIns, int numOuts, int frameSize) {
            array2sh_process(hA2sh, inFrame, outFrame, numIns, numOuts, frameSize);
        });
}

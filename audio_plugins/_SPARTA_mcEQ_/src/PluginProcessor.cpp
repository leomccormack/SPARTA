    ...
    switch (nCurrentBlockSize) {
        case (FRAME_SIZE * 8):
            for (int frame = 0; frame < 8; frame++) {
                ...
                mceq_process(hMEQ, ringBufferInputs, ringBufferOutputs, nNumInputs, nNumOutputs, FRAME_SIZE, (int)isPlaying);
        ...
        case FRAME_SIZE:
            mceq_process(hMEQ, bufferData, outputs, nNumInputs, nNumOutputs, FRAME_SIZE, (int)isPlaying);
            buffer.clear();
            for (int ch = 0; ch < nNumOutputs; ch++) {
                for (int i = 0; i < FRAME_SIZE; i++) {

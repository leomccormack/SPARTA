...
private:
    ...
    /* 2d slider handles */
    ScopedPointer<log2dSlider> balance2dSlider;
    ScopedPointer<log2dSlider> decBalance2dSlider;

    /* sofa file loading */
    void filenameComponentChanged (FilenameComponent*) override  {
        String directory = fileChooser.getCurrentFile().getFullPathName();
        const char* new_cstring = (const char*)directory.toUTF8();
        ambi_cropac_setSofaFilePath(hVst->hAmbi, new_cstring);
    }
    ...
    std::unique_ptr<Slider> s_decBal;

...
void inputCoordsView::sliderValueChanged (Slider* sliderThatWasMoved)
{
    ...
    for(int i=0; i<maxNCH; i++){
        if (sliderThatWasMoved == aziSliders[i]) {
            ambi_enc_setSourceAzi_deg(hVst->hAmbi, i, (float)aziSliders[i]->getValue());
            break;
        }
        if (sliderThatWasMoved == elevSliders[i]) {
            ambi_enc_setSourceElev_deg(hVst->hAmbi, i, (float)elevSliders[i]->getValue());
            break;
        }
    }
...
void inputCoordsView::refreshCoords(){
    ...
    for( int i=0; i<maxNCH; i++){
        ...
        aziSliders[i]->setValue(ambi_enc_getSourceAzi_deg(hVst->hAmbi, i), dontSendNotification);
        elevSliders[i]->setRange (-180.0, 180.0, 0.001);
        elevSliders[i]->setValue(ambi_enc_getSourceElev_deg(hVst->hAmbi, i), dontSendNotification);
    }
}

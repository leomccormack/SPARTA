...
static juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout()
{
    ...
    params.push_back(std::make_unique<juce::AudioParameterChoice>("beamType", "BeamType", juce::StringArray{"Cardioid", "Hyper-Card", "Max-EV"}, 1));
    params.push_back(std::make_unique<juce::AudioParameterInt>("numBeams", "NumBeams", 1, MAX_NUM_OUTPUTS, beamformer_defaultNumBeams, AudioParameterIntAttributes().withAutomatable(false)));
    for(int i=0; i<MAX_NUM_OUTPUTS; i++){
        params.push_back(std::make_unique<juce::AudioParameterFloat>("azim" + juce::String(i), "Azim_" + juce::String(i+1), juce::NormalisableRange<float>(-180.0f, 180.0f, 0.01f),
                                                                     beamformer_defaultBeamDirections[i][0],
                                                                     AudioParameterFloatAttributes().withLabel(juce::String::fromUTF8(u8"\u00B0"))));
        params.push_back(std::make_unique<juce::AudioParameterFloat>("elev" + juce::String(i), "Elev_" + juce::String(i+1), juce::NormalisableRange<float>(-90.0f, 90.0f, 0.01f),
                                                                     beamformer_defaultBeamDirections[i][1],
                                                                     AudioParameterFloatAttributes().withLabel(juce::String::fromUTF8(u8"\u00B0"))));
    }
...
void PluginProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    blockAdapter->configure(beamformer_getFrameSize(), nNumInputs, nNumOutputs, nHostBlockSize);
}
...
void PluginProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& /*midiMessages*/)
{
    ...
    blockAdapter->processBlock (buffer, [this] (const float* const* inFrame, float* const* outFrame, int numIns, int numOuts, int frameSize) {
            beamformer_process(hBeam, inFrame, outFrame, numIns, numOuts, frameSize);
        });
}
